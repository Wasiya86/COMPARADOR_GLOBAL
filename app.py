    ...
    if len(cp) < 2:
        ...
        if zona_info.empty:
            ...
            if z_cbl == "Canarias" or z_cbl == "Especial":
                st.warning("⚠️ Este destino es Insular/Especial. Por favor, usa el Excel de Canarias para los costes de aduanas.")
            else:
                costes = {}
                ...
                if all(v == float('inf') for v in costes.values()):
                    st.error("No hay agencias disponibles para ese peso.")
                else:
